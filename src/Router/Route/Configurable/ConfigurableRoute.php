...
use Concept\Http\Router\Route\Configurable\Handler\RouteHandlerInterface;
use Psr\Http\Message\ResponseInterface;
use Psr\Http\Message\ServerRequestInterface;

class ConfigurableRoute implements ConfigurableRouteInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        return $this->getRouteHandlerPrototype()
            ->withConfig($this->getConfig()->fromPath(ConfigurableRouteInterface::CONFIG_HANDLER_NODE))
            ->handle($request);
    }
    ...
    {
        if (!$this->getConfig()->has(ConfigurableRouteInterface::CONFIG_METHOD_NODE)) {
            throw new \RuntimeException('Route methods are not set');
        }

        $method =  $this->getConfig()->get(ConfigurableRouteInterface::CONFIG_METHOD_NODE);
    ...
    {
        if (!$this->getConfig()->has(ConfigurableRouteInterface::CONFIG_PATH_NODE)) {
            throw new \RuntimeException('Route path is not set');
        }

        return $this->getConfig()->get(ConfigurableRouteInterface::CONFIG_PATH_NODE);
    }
